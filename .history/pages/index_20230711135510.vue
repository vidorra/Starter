				...
				<div>  
					...
					>
						<div>
							...
							<ul v-if="b.children" class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8 mt-2">
							...
							<li class="list-item bg-background rounded-xl text-default text-fontcolor font-semibold hover:text-primary-900 underline underline-offset-4 decoration-wavy decoration-primary/40 hover:decoration-primary transition-all">
								...
								<NuxtLink class="text-xl" :to="`${child._path}`">
    								                <img :src="getImageSrc(k, b.children)" @error="useDefaultImage" class="mb-4 rounded-md mt-2" alt="Blog Image" />
									{{ child.title }}
								</NuxtLink>
								</div>
							</li>
