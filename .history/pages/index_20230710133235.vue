...
<script setup>
 ...
 const getChildImage = async (child) => {
  const rawChild = toRaw(child);
  console.log('Child:', rawChild);

  // Retrieve the Markdown content for the child
  const content = await getContent(rawChild._path);

  // Extract the image path from the Markdown content
  const regex = /image:\s+(.*)/;
